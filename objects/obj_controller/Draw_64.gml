...
if (global.DEBUG_DRAW_MINIMAP) {
	...
	if (instance_exists(obj_road_generator)) {
		...
		var target = main_camera_target;
		var cp = find_nearest_cp(target.x, target.y, target.on_road_index._id div obj_road_generator.road_segments);
		var ri_start = max(0, (cp - 4) * obj_road_generator.road_segments);
